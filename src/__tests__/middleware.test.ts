...
async function makeRequest(pathname: string, userAgent: string, originHtml: string = '<html lang="en"><head><title>Default</title></head><body></body></html>') {
...
describe('middleware: bot detection', () => {
  ...
  it('processes Twitter bot requests', async () => {
    const { response } = await makeRequest('/', 'TwitterBot/1.0', '<html lang="en"><head><title>Default</title></head></html>');
    expect(response).not.toBeUndefined();
  });

  it('processes Facebook bot requests', async () => {
    const { response } = await makeRequest('/blog', 'facebookexternalhit/1.0', '<html lang="en"><head></head></html>');
    expect(response).not.toBeUndefined();
  });

  it('processes Google bot requests', async () => {
    const { response } = await makeRequest('/grants', 'GoogleBot/2.1', '<html lang="en"><head></head></html>');
    expect(response).not.toBeUndefined();
  });

  it('bypasses non-bot requests', async () => {
    const { response } = await makeRequest('/', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36', '<html lang="en"><head></head></html>');
    expect(response).toBeUndefined();
  });

  it('bypasses non-bot requests on all routes', async () => {
    const { response } = await makeRequest('/blog/my-post', 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)', '<html lang="en"><head></head></html>');
    expect(response).toBeUndefined();
  });
});
...
describe('middleware: non-indexable routes', () => {
  ...
  it('bypasses bot requests for non-indexable routes', async () => {
    const { response } = await makeRequest('/admin', 'TwitterBot/1.0', '<html lang="en"><head></head></html>');
    expect(response).toBeUndefined();
  });

  it('bypasses bot requests for unknown routes', async () => {
    const { response } = await makeRequest('/nonexistent', 'TwitterBot/1.0', '<html lang="en"><head></head></html>');
    expect(response).toBeUndefined();
  });

  it('bypasses bot requests for /settings', async () => {
    const { response } = await makeRequest('/settings', 'TwitterBot/1.0', '<html lang="en"><head></head></html>');
    expect(response).toBeUndefined();
  });
});
...
describe('middleware: meta tag injection', () => {
  ...
  it('injects correct description for home route', async () => {
    const { response } = await makeRequest('/', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
  ...
  it('injects og:title for blog route', async () => {
    const { response } = await makeRequest('/blog', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
    expect(html).toContain('Blog — Wraith Protocol');
  });

  it('injects og:image for grants route', async () => {
    const { response } = await makeRequest('/grants', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
  ...
  it('injects og:image for stellar route', async () => {
    const { response } = await makeRequest('/stellar', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
    expect(html).toContain('og/stellar.png');
  });

  it('injects og:image for roadmap route', async () => {
    const { response } = await makeRequest('/roadmap', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
    expect(html).toContain('og/roadmap.png');
  });

  it('injects og:image for use-cases route', async () => {
    const { response } = await makeRequest('/use-cases', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
    expect(html).toContain('og/use-cases.png');
  });

  it('injects og:url for blog post route', async () => {
    const { response } = await makeRequest('/blog/wave-7-kickoff', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
  ...
  it('injects og:image for case study route', async () => {
    const { response } = await makeRequest('/case-studies/payroll-processor', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
...
describe('middleware: localization', () => {
  ...
  it('injects Spanish title for /es/stellar', async () => {
    const { response } = await makeRequest('/es/stellar', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
  ...
  it('injects Spanish title for /es/grants', async () => {
    const { response } = await makeRequest('/es/grants', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
  ...
  it('injects og:locale es_ES for Spanish', async () => {
    const { response } = await makeRequest('/es/', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
    expect(html).toContain('og:locale" content="es_ES"');
  });

  it('handles /en/ prefix correctly', async () => {
    const { response } = await makeRequest('/en/stellar', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
  ...
  it('handles /es/blog/:slug with Spanish locale', async () => {
    const { response } = await makeRequest('/es/blog/wave-7-kickoff', 'TwitterBot/1.0', '<html lang="en"><head><title>X</title></head></html>');
    const html = await response!.text();
    expect(html).toContain('og:image" content="https://usewraith.xyz/og/blog-wave-7-kickoff.png"');
    expect(html).toContain('lang="es"');
    expect(html).toContain('og:locale" content="es_ES"');
  });
});
...
describe('middleware: existing behavior preservation', () => {
  ...
  it('preserves original roadmap behavior', async () => {
    const { response } = await makeRequest('/roadmap', 'Slackbot 1.0', '<html lang="en"><head><title>Default</title></head></html>');
    const html = await response!.text();
  ...
  it('preserves original stellar behavior', async () => {
    const { response } = await makeRequest('/stellar', 'LinkedInBot/1.0', '<html lang="en"><head><title>Default</title></head></html>');
    const html = await response!.text();
    expect(html).toContain('Stellar Integration');
    expect(html).toContain('og/stellar.png');
  });
  ...
  it('preserves original usecases behavior for /use-cases', async () => {
    const { response } = await makeRequest('/use-cases', 'GoogleBot/2.1', '<html lang="en"><head><title>Default</title></head></html>');
    const html = await response!.text();
...
describe('middleware: HTML escaping', () => {
  ...
  it('escapes ampersands in meta descriptions', async () => {
    const { response } = await makeRequest('/blog/wave-7-kickoff', 'TwitterBot/1.0', '<html lang="en"><head></head></html>');
    const result = await response!.text();
