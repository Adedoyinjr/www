...
const STATIC_ROUTES: Record<string, {
  title: string;
  description: string;
  ogImage: OgImageConfig;
  ogType: 'website' | 'article';
  ...
  '/newsletter': {
    ...
    description:
      'Mainnet updates, security advisories, and grant news — no tracking.',
    ogImage: {
      title: 'Newsletter',
  ...
  '/stellar': {
    ...
    description:
      'Native stealth address support on the Stellar network with memo-based metadata.',
    ogImage: {
...
const blogManifestData = (blogManifest as Array<{
  slug: string;
  title: string;
  excerpt: string;
}>) || [];
...
export function ogImageSlugFor(pathname: string, _locale: Locale): string | null {
  const { pathname: cleanPath } = stripLocalePrefix(pathname);
  ...
  const isDynamicBlog = cleanPath.match(/^\/blog\/(.+)$/);
  if (isDynamicBlog) {
    return `blog-${isDynamicBlog[1]}`;
  }

  const isDynamicCaseStudy = cleanPath.match(/^\/case-studies\/(.+)$/);
  if (isDynamicCaseStudy) {
    return `case-study-${isDynamicCaseStudy[1]}`;
  }
